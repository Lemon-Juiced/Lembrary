...
public enum LembraryArmorMaterials implements ArmorMaterial {
    /** Vanilla Armor Tiers:
    ...
    NECRONITE("necronite", 42, Util.make(new EnumMap<>(ArmorItem.Type.class), (protectionFunctionForType) -> {
        ...
        return Ingredient.of(TagRegistry.Items.INGOTS_NECRONITE);
    }),
    UMBRALITE("umbralite", 50, Util.make(new EnumMap<>(ArmorItem.Type.class), (protectionFunctionForType) -> {
        ...
        return Ingredient.of(TagRegistry.Items.GEMS_UMBRALITE);
    });
    ...
    private LembraryArmorMaterials(String name, int durabilityMultiplier, EnumMap<ArmorItem.Type, Integer> protectionFunctionForType, int enchantmentValue, SoundEvent sound, float toughness, float knockbackResistance, Supplier<Ingredient> repairIngredient) {
        this.name = name;
    ...
    public String getName() {
        return Lembrary.MOD_ID + ":" + this.name;
    }
